...
type TypedProvider = {
  [CopilotProviderType.Google]: GoogleProvider;
...
export class CopilotProviderFactory {
  ...
  async getProviderByCapability<C extends CopilotCapability>(
    ...
    for (const [type, provider] of this.#providers.entries()) {
      ...
      if (provider.capabilities.includes(capability)) {
        ...
        // then we match by model and prefer filter
        if (
  ...
  async getProviderByModel(
    ...
    return candidate as CapabilityToCopilotProvider[keyof CapabilityToCopilotProvider];
  }
