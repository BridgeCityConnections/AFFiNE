...
export class AudioAttachmentBlock extends Entity<AttachmentBlockModel> {
  ...
  constructor(readonly audioMediaManagerService: AudioMediaManagerService) {
    super();
  ...
  mount() {
    ...
    // if the transcription job's creator is the current user, we should auto start the transcription job
  ...
  readonly transcribe = () => {
    try {
      ...
      if (this.transcriptionJob.status$.value.status === 'started') {
        return;
      }
      this.transcriptionJob
        .start()
        .then(() => {
          const status = this.transcriptionJob.status$.value;
  ...
  private readonly fillTranscriptionResult = (result: TranscriptionResult) => {
    this.props.props.caption = result.summary ?? '';
